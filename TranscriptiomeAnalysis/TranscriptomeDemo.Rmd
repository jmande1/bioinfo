...
# Frames the phenotypic data from the reference metadata
```{r}
...
# filtering the Ballgown object and reassessing the tranrscript number
```{r}
...
# choose a transcript to examine more closely (chose gene-PA2072)
```{r}
results_transcripts[results_transcripts$transcriptNames == "gene-PA2072", ]
```
## From this transcript above, you're given the description of the gene (in this case it's a transcript, or an RNA sequence), its given identification number, the fold change value (fc) that detects differentially expressed genes, p-value (that statistically checks likelihood of this sequence being significantly different), and the q-value (or adjusted p-values)

# filters the sequences that have a p-value less than 0.05, showing significantly different data
```{r}
sigdiff <- results_transcripts %>% filter(pval<0.05)
dim(sigdiff)
```

# organize the table <the table is organized by p-value and fc metrics> #what are the metrics that this table is organized by?
```{r}
...
# Takes the output (the table above) and categorizes them by column name, and checks the dimensions of the data <hint:compare the above output of head(gene_expression) to this output>
```{r}
...
## There are more genes that express one transcript versus genes that express more than one transcript. 
...
min_nonzero=1
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="darkblue", cex=0.25,
...
# If the two data sets are similar, I did something wrong with the code then, because I was then comparing the same sets of data
...
## interpret the above figure: The fold-change (fc) that are greater than 4 occur at frequencies less than 2, between Planktonic versus Biofilm transcripts. 
...
## There is a significant difference between the RNA expression between biofilm and planktonic. 