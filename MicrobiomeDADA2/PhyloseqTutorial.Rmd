...
```{r}
dna <- Biostrings::DNAStringsSet(taxa_names(physeq))
...
physeq_relabund <- transform_sample_counts(physeq, function(x) x / sume(x))

# barplot
plot_bar(physeq_relabund, fill = "Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="fill") + facet_wrap(~body.sit, scales = "free")
```
...
test simpson <- wilcox.text(Simpson ~ subject, data = alpha)
test shannon <- wilcox.text(Shannon ~ subject, data = alpha)
